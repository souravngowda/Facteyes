...
async def main():
    ...
    while True:
        ...
        # Send waiting message if no data was sent and 10 seconds have passed
        if not data_sent and (current_time - last_waiting_message_time) >= 10:
            waiting_message = {
                "MessageType": "STATUS",
                "Status": "Waiting for new row data",
                "Timestamp": Currenttimestamp
            }
            await device_client.send_message(json.dumps(waiting_message))
            print("[STATUS] Sent: Waiting for new row data")
