...
def convert_cycletime_to_minutes(cycletime_str):
    """Convert cycle time from 'X min Y.YY sec' to decimal minutes format."""
    match = re.match(r"(?:(\d+)\s*min\s*)?(?:(\d+\.\d+|\d+)\s*sec)?", str(cycletime_str))
    if match:
        minutes = int(match.group(1)) if match.group(1) else 0
        seconds = float(match.group(2)) if match.group(2) else 0
        return round(minutes + (seconds / 60), 4)  # Convert seconds to minutes
    return None
...
async def send_product_status(device_client, processed_barcodes, rows):
    ...
    for row in rows:
        ...
        if orgbarcode in processed_barcodes:
            ...
            Shift = row[13]
            Cycletime = row [16]
            ConvertedCycletime = convert_cycletime_to_minutes(Cycletime) if Cycletime else 0.0            
            ...
            ProductStatusData = {
                ...
                "ProductionStartDate": Testdate.strftime("%Y-%m-%dT%H:%M:%SZ"),
                "ProductionEndDate": Testdate.strftime("%Y-%m-%dT%H:%M:%SZ"),
                "MessageType": "MICROLOGICPRODUCTS",
                "OPERATOR": Operator,
                "Shift": Shift,
                "OverallCycletime":ConvertedCycletime,                
